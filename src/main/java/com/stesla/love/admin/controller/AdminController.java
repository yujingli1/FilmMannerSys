...
public class AdminController {
    ...
    public Map<String, Object> info() {
        try {
            ...
            return Result.success(map1).getMap();
        } catch (Exception e) {
            return Result.error(401, "Token error!").getMap();
        }
    }
        ...
        try {
            ...
            if(map.containsKey("newPassword")){  // 校验新密码
                ...
                if(newPassword.length() < 5 || newPassword.length() > 16) {  // 校验新密码长度
                    return Result.error(400, "New password must be 5-16 characters long!").getMap();
                }
            ...
            if(map.containsKey("email")){  // 校验邮箱
                ...
                if(!email.matches(emailReg)) {
                    return Result.error(400, "Email format error!").getMap();
                }
            ...
            if(map.containsKey("fullName")){  // 校验全名
                ...
                if(fullName.length() < 2 || fullName.length() > 16) {  // 校验全名长度
                    return Result.error(400, "Full name must be 2-16 characters long!").getMap();
                }
            ...
            return Result.error(401, e.getMessage()).getMap();
        }
    }

    @PostMapping("/test")
    public Map<String, Object> test() {
        return Result.success("我列个豆测试成功").getMap();
    }
}
